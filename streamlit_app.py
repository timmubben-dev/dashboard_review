...
st.set_page_config(page_title="Herzklappen Dashboard", layout="wide")
st.title("🏥 Herzklappen Dashboard Generator")
st.info("Laden Sie Ihre Excel-Datei hoch, um das automatisierte Dashboard zu generieren.")

uploaded_file = st.file_uploader("Wählen Sie die 'Klappenzahlen.xlsx' aus", type=["xlsx"])
    ...
    try:
        ...
        with pd.ExcelWriter(output, engine='xlsxwriter') as writer:
            ...
            for r, y in enumerate([2024, 2025, 2026]):
                y_df = df[df['Year'] == y]
                v_all = y_df[(y_df['VWD_num'] > 0) & (y_df['VWD_num'] < 100)]['VWD_num']
                v_short = y_df[(y_df['VWD_num'] > 0) & (y_df['VWD_num'] < 21)]['VWD_num']
                ws.write(12+r, 0, y, cell_f)
                ws.write(12+r, 1, v_all.median() if not v_all.empty else 0, cell_f)
                ws.write(12+r, 2, v_short.mean() if not v_short.empty else 0, num_f)
                med_short = v_short.median() if not v_short.empty else 0
                ws.write(12+r, 3, med_short, green_f if 0 < med_short <= 5 else red_f)

            # 3. KLAPPENSPRECHSTUNDE
            ws.write('A17', '3. ZUWEISUNG ÜBER KLAPPENSPRECHSTUNDE', title_f)
            ...
            for r, y in enumerate([2025, 2026]):
                ...
                ws.write(18+r, 0, y, cell_f); ws.write(18+r, 1, ks_count, cell_f)
            ...
            for r, row in enumerate(team_stats.values):
                ...
                ws.write(24+r, 2, row[1]/total_tavi if total_tavi > 0 else 0, pct_f)
            ...
            ev_fmt = green_pct_f if ev_r >= 0.8 else red_pct_f if ev_r < 0.7 else yellow_pct_f
            ws.write(33, 1, ev_r, ev_fmt)

            # Spaltenbreiten
            ws.set_column('A:A', 36); ws.set_column('B:M', 6.8); ws.set_column('N:Q', 13)

        st.success("Dashboard wurde erstellt!")
        st.download_button(
        ...
        st.error(f"Fehler: {e}. Prüfen Sie, ob das Tabellenblatt 'Daten' heißt.")
